...
배경은 완전한 검정색을 사용한다.
글자와 미디아는 화면 상단에서 15% 떨어진 곳에 상단정렬(9:16 일 때, 미디아 상단 기준 위치 측정으로 화면 상단 15%)이나 미디아 기준 화면의 중앙(16:9 일 때, 미디아 중앙 기준 화면의 중앙정렬)에 정렬한다.
...
나레이션 생성 시 자신이 가지고 있는 tts도구 또는 SKILLS를 이용하며, gTTS/파이썬 기본 TTS등을 이용하는 것은 금지된다.(품질 문제 때문이다.)
### 4.4. 출력
사용자에게 영상을 mp4로 제공한다.
