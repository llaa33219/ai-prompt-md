...
Playwright 또는 브라우저 스크린샷 도구 등이 있는 경우, 그것을 사용하여 이미지를 얻는다.
### 3.4. 검증
이 문서에 나온 svg와 동일한 형식으로 제작하였는지 한 번 더 살펴보고, 실제 이미지, 스크린샷이 들어갔으며 이미지 생성으로 생성된 이미지가 들어가지 않았는지 검증한다.
문제가 발견되었으면 수정하여야 한다.
### 3.4. 출력
mp4 파일로 사용자에게 제공한다.
